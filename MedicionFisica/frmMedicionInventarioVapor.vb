...
'Fecha: 20/01/2006
'Motivo: La medición de la densidad por hidrometro puede ser antes o despues de la toma del
'inventario inicial
'Variable de cambio: 20060120CAGP$001
...
Public Class frmMedicionInventarioVapor
    ...
    Private _NumEmpleadoTanque As Integer
    Private _NumEmpleadoHidrometro As Integer
    Private _MedidorDensidad As Integer
    ...
    Private _CalculargasVapor As Boolean = True
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmMedicionInventarioVapor))
        ...
        Me.cboHDTemperatura.TabIndex = 16
        '
        'txtHDTemperatura
        '
        ...
        Me.txtHDTemperatura.TabIndex = 15
        ...
        Me.Name = "frmMedicionInventarioVapor"
...
#Region "Metodos y Funciones"
    Public Sub InicializaForma(ByVal Operacion As Short, ByVal NumeroTanque As String, _
                               ByVal Tanque As Integer, ByVal CapacidadTanque As Integer, _
                               ByVal Transportadora As Short, ByVal AlmacenGas As Integer, _
                               ByVal UsuarioAlta As String, ByVal Empleado As Integer, _
                               Optional ByVal TipoLectura As String = "", Optional ByVal NumEmpleadoTanque As Integer = 0, _
                               Optional ByVal NumEmpleadoHidrometro As Integer = 0, Optional ByVal MedidorDensidad As Integer = 0, _
                               Optional ByVal Fecha As String = "", Optional ByVal PorcentajeDefault As String = "", _
                               Optional ByVal Sucursal As Short = 0, Optional ByVal Corporativo As Short = 0)
        If Sucursal = 0 Then
            Sucursal = MedicionFisica.Globals.GetInstance._Sucursal
        End If
        If Corporativo = 0 Then
            Corporativo = MedicionFisica.Globals.GetInstance._Corporativo
        End If

        Dim oConfig As New SigaMetClasses.cConfig(16, Corporativo, Sucursal)
        ...
        _CalculargasVapor = CType(oConfig.Parametros("CalcularGasVapor"), Boolean)
        ...
        _NumEmpleadoTanque = NumEmpleadoTanque
        _NumEmpleadoHidrometro = NumEmpleadoHidrometro
        _MedidorDensidad = MedidorDensidad
        _Fecha = Fecha
        _PorcentajeDefault = PorcentajeDefault

        If _TipoLectura = "INVENTARIO" Then
            _TemperaturayPresion = True
            _TemperaturayPresion = _CalculargasVapor
            gpbHidrometro.Enabled = _CalculargasVapor
        End If
    End Sub

    Function ValidarValoresEmbarque() As Boolean
        ' 20060120CAGP$001 /I
        'If dtpFHoraInicial.Value > dtpHDFHora.Value Then
    ...
    Function ValidarValoresPageEmbarque() As Boolean
        ...
        ElseIf cboEmpleadoInicial.SelectedIndex = -1 Or txtNominaInicial.Text.Length = 0 Then
            ...
            Return False
        ElseIf cboMedidor.SelectedIndex = -1 Then
            Dim Mensaje As PortatilClasses.Mensaje
            ...
            End If
            Return False
        ElseIf _CalculargasVapor And txtHDDensidad.Text.Length = 0 Then 'CAGP
            Dim Mensaje As PortatilClasses.Mensaje
            Mensaje = New PortatilClasses.Mensaje(59)
            MessageBox.Show(Mensaje.Mensaje, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
            ActiveControl = txtHDDensidad
            Return False
        ElseIf _CalculargasVapor And txtHDPresion.Text.Length = 0 Then
            Dim Mensaje As PortatilClasses.Mensaje
            Mensaje = New PortatilClasses.Mensaje(57)
            MessageBox.Show(Mensaje.Mensaje, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
            ActiveControl = txtHDPresion
            Return False
        ElseIf _CalculargasVapor And txtHDTemperatura.Text.Length = 0 Then
            Dim Mensaje As PortatilClasses.Mensaje
            Mensaje = New PortatilClasses.Mensaje(58)
            MessageBox.Show(Mensaje.Mensaje, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub CargarDatos()
        If _Operacion = 0 And Not _CapturaDatos Then
            ...
            If Not _MoverMedidorDensidad Then
                cboMedidor.PosicionaCombo(_MedidorDensidadDefault)
                cboMedidor.Enabled = _MoverMedidorDensidad
            Else
                If _MedidorDensidad > 0 Then
                    cboMedidor.PosicionaCombo(_MedidorDensidad)
                    cboMedidor.Enabled = _MoverMedidorDensidad
                Else
                    cboMedidor.PosicionaCombo(_MedidorDensidadDefault)
                    cboMedidor.Enabled = _MoverMedidorDensidad
            ...
            If _NumEmpleadoTanque <> 0 Then
                cboEmpleadoInicial.SelectedValue = _NumEmpleadoTanque
                txtNominaInicial.Text = CType(_NumEmpleadoTanque, String)
            Else
                cboEmpleadoInicial.SelectedIndex = -1
                cboEmpleadoInicial.SelectedIndex = -1
            End If

            If _NumEmpleadoHidrometro <> 0 Then
                cboHDEmpleado.SelectedValue = _NumEmpleadoHidrometro
                txtHDNomina.Text = CType(_NumEmpleadoHidrometro, String)
            Else
                cboHDEmpleado.SelectedIndex = -1
                cboHDEmpleado.SelectedIndex = -1
            End If
            ...
            Else
                ...
                dtpHDFHora.Value = CType(_Fecha, DateTime)
            End If
            ...
            If Not _TemperaturayPresion Then
                ...
                If _CalculargasVapor = False Then
                    lblHDPresion.Font = New Drawing.Font(lblPresionInicial.Font, FontStyle.Regular)
                    lblTemperatura.Font = New Drawing.Font(lblTemperaturaInicial.Font, FontStyle.Regular)
                End If
            End If
    ...
    Public Sub AlmacenarInformacion(ByVal MovimientoAlmacen As Integer, ByVal TipoLectura As String)
        ...
        If RealizarMovimiento Then
            ...
            If cboHDEmpleado.Text = "" Then
                EmpleadoHD = _Empleado
            Else
            ...
            Else
                TemperaturaHD = oMedicionFisicaConvierteTemperatura.CovierteTemperaturaCentigrados(CType(txtHDTemperatura.Text, Decimal), cboHDTemperatura.SelectedIndex)
            End If
            If txtHDPresion.Text.Length = 0 Then
                PresionHD = 666
            Else
                PresionHD = CType(txtHDPresion.Text, Decimal)
            End If
            ...
            Dim oMedicionFisica As New PortatilClasses.CatalogosPortatil.cMedicionFisica(1, 0, "", _UsuarioAlta, _AlmacenGas, MovimientoAlmacen, 0, 0, DensidadHD, _Empleado, CType(dtpFHoraInicial.Value, String), "ACTIVO", TipoLectura)
            ...
            Dim oMedicionFisicaCorte As New PortatilClasses.Consulta.cMedicionFisicaCorte(2, oMedicionFisica.MedicionFisica)
...
#End Region
    ...
    Private Sub cboTemperaturaInicial_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cboTemperaturaInicial.KeyDown, dtpFHoraInicial.KeyDown, cboHDTemperatura.KeyDown, dtpHDFHora.KeyDown, cboMedidor.KeyDown
    ...
    Private Sub txtHDPresion_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtHDPresion.Leave
        If CType(sender, TextBox).Text.Length > 0 Then
