...
Public Class frmRemisionManual
	...
	Private _BoletinEnLineaCamion As Boolean
	...
	Public Property ZonaEconomicaActual As Integer
		Get
			Return _ZonaEconomicaActual
		End Get
		Set(value As Integer)
			_ZonaEconomicaActual = value
		End Set
	End Property
	...
	Public Property LiqPrecioVigente As Boolean
		Get
			Return _LiqPrecioVigente
		End Get
		Set(value As Boolean)
...
#Region " Windows Form Designer generated code "
	...
	Public Sub New(ByVal FolioAtt As Integer,
		...
		_cargarProductosPadre = CargarProductosPadre
	...
	Public Sub New(ByVal FolioAtt As Integer,
				   ...
				   ByVal DtRemisiones As DataTable,
				   ByVal Cliente As Integer,
				   ByVal rutaMovilgas As Boolean,
				   Optional ByVal ProductosPadre As DataTable = Nothing,
				   Optional CargarProductosPadre As Boolean = False)
		',ByVal DtProductos As DataTable)
		MyBase.New()
		_BoletinEnLineaCamion = rutaMovilgas
...
#End Region
	...
	Private Sub CargarProductosVarios()
		...
		If _dtProductos.Rows.Count > 5 Then
	...
	'Inicializa los valores de cada label y textbox que se crearán dinámicamente
	...
	Public Sub AddControls(ByVal Descripcion As String, ByVal Existencia As Integer,
		ByVal ylbl As Integer, ByVal ylbl2 As Integer,
								   ByVal ytxt As Integer, ByVal texboxActivo As Boolean)
	...
	Private Function VerificaDatos() As Boolean
		...
		Return Flag
	End Function
	Private Function ValidaZonaEconomica(ByVal ZonaEconomicaTemp As Integer) As Boolean
		If _ZonaEconomicaActual = -1 Then
			Return True
		Else
			Return _ZonaEconomicaActual = ZonaEconomicaTemp
		End If
						...
						If dtLiquidacionTotal.TableName = "LiquidacionTotal" Then
							...
							If cbxAplicaDescuento.Checked Then
								Try
									...
									Descuento = calculaDescuento(TipoDescuento, Descuento, CType(_dtProductos.Rows(i).Item(5), Decimal), CType(_dtProductos.Rows(i).Item(5), Decimal))
						...
						Else 'Tabla Remision
							...
							If cbxAplicaDescuento.Checked Then
								Try
									Descuento = CDec(_DatosCliente.GetValue(7))
									TipoDescuento = CInt(_DatosCliente.GetValue(6))
									Descuento = calculaDescuento(TipoDescuento, Descuento, CType(_dtProductos.Rows(i).Item(2), Decimal), CType(_dtProductos.Rows(i).Item(2), Decimal))
	...
	Private Sub BorrarGridPedido()
		...
		End If
		Totaliza()
		If CType(grdDetalle.DataSource, DataTable).Rows.Count = 0 Then
			_ZonaEconomicaActual = -1
		End If
		...
		If _DetalleGrid IsNot Nothing AndAlso _DetalleGrid.Rows.Count > 0 Then
			...
			If _DetalleGrid.Rows.Count > 0 Then
				For Each DR As DataRow In _DetalleGrid.Rows
					...
					_TotalLiquidarPedido = _TotalLiquidarPedido + CType(DR(6), Decimal)
				Next
			End If
			lblTotalKilos.Text = CType(_Kilos, Decimal).ToString("N2")
			lblTotal.Text = CType(_TotalLiquidarPedido, Decimal).ToString("N2")
	...
	Private Sub frmRemisionManual_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		If (_ZonaEconomicaActual <> -1) Then
			cboZEconomica.SelectedIndex = cboZEconomica.FindString(_ZonaEconomica)
		End If


		If _cargarProductosPadre Then
			CargarProductosVarios_Padre()
		Else
			CargarProductosVarios()
		End If
		...
		'AddHandler cboZEconomica.Leave, AddressOf cboZEconomica_Leave
	...
	Private Sub Btn_Modificar_Click(sender As Object, e As EventArgs) Handles Btn_Modificar.Click
		...
		If Not ValidaZonaEconomica(cboZEconomica.Identificador) Then
			MessageBox.Show("No se puede usar otra zona economica, solamente " & _ZonaEconomica, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
		Else
			CargaGridModificado()
			_ZonaEconomica = cboZEconomica.Text
			_ZonaEconomicaActual = cboZEconomica.Identificador
	...
	Private Sub grdDetalle_CurrentCellChanged(sender As Object, e As EventArgs) Handles grdDetalle.CurrentCellChanged
		Try
			...
			If grdDetalle.VisibleRowCount > 0 Then
				...
				Dim objCliente As New PortatilClasses.Consulta.cCliente(0, CType(TxtCliente.Text, Integer))
				objCliente.CargaDatos()
				lblNombreCliente.Text = objCliente.Cliente
				AsignarDatosCliente(objCliente.IdCliente)
	...
	Public Function calculaDescuento(tipoDescuento As Integer, valorDescuento As Decimal, cantidadProducto As Decimal, kilos As Decimal) As Decimal
		Dim _descuento As Decimal = 0

		Select Case tipoDescuento
			Case 1
				_descuento = (cantidadProducto * valorDescuento)
			Case 5
				_descuento = valorDescuento
			Case 6
				_descuento = (kilos * valorDescuento)
		End Select
		...
		Try
			...
			If grdDetalle.VisibleRowCount > 0 Then
				If _RutaMovil = True Then
					...
					Dim campoKilosTmp As Integer = CType(grdDetalle.DataSource, DataTable).Columns.IndexOf("Kilos")
					Dim campoCantidadTmp As Integer = CType(grdDetalle.DataSource, DataTable).Columns.IndexOf("Cantidad")

					Dim KilosTmp As Decimal = CType(grdDetalle.Item(i, campoKilosTmp), Decimal)
					Dim CantidadTmp As Decimal = CType(grdDetalle.Item(i, campoCantidadTmp), Decimal)
					...
					If cboTipoCobro.Identificador = 18 And CBool(_DatosCliente.GetValue(3)) = True Then
						...
						If cbxAplicaDescuento.Checked Then
							Try
								Descuento = CDec(_DatosCliente.GetValue(7))
								TipoDescuento = CInt(_DatosCliente.GetValue(6))
								Descuento = calculaDescuento(TipoDescuento, Descuento, CType(_dtListaProductos.Rows(fila).Item(5), Decimal), KilosTmp / CantidadTmp)
							...
							If cboTipoCobro.Identificador = 17 And CBool(_DatosCliente.GetValue(4)) = True Then
								...
								If cbxAplicaDescuento.Checked Then
									Try
										...
										'descuentoGrupal = CType(grdDetalle.Item(i, 10), Decimal) * Descuento
										Descuento = calculaDescuento(TipoDescuento, Descuento, CType(grdDetalle.Item(i, 4), Decimal), KilosTmp / CantidadTmp)
							...
							If cboTipoCobro.Identificador = 18 And CBool(_DatosCliente.GetValue(3)) = True Then
								...
								If cbxAplicaDescuento.Checked Then
									Try
										Descuento = CDec(_DatosCliente.GetValue(7))
										TipoDescuento = CInt(_DatosCliente.GetValue(6))
										Descuento = calculaDescuento(TipoDescuento, Descuento, CType(_dtListaProductos.Rows(fila).Item(5), Decimal), CType(_dtListaProductos.Rows(fila).Item(5), Decimal))
									...
									If cboTipoCobro.Identificador = 17 And CBool(_DatosCliente.GetValue(4)) = True Then
										...
										If cbxAplicaDescuento.Checked Then
											Try
												...
												TipoDescuento = CInt(_DatosCliente.GetValue(6))
												descuentoGrupal = calculaDescuento(TipoDescuento, Descuento, CType(grdDetalle.Item(i, campoCantidad), Decimal) * CType(_dtListaProductos.Rows(fila).Item(2), Decimal), CType(grdDetalle.Item(i, campoCantidad), Decimal) * CType(_dtListaProductos.Rows(fila).Item(5), Decimal))
									...
									Else
										...
										Else
											...
											Else
												...
												If cboTipoCobro.Text.Trim = "Obsequio" Then
													grdDetalle.Item(i, 7) = CType(0, Decimal)
