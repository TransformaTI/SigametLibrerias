...
function doitmd()
{
	...
	if(!(it.getpar().isclick&&it.getsub()))
	{
		...
		{				
		    //MODULO DE CATALOGOS	
			if(it.url=="./CatalogoCaja.aspx"){
				self.parent.navigate('./CatalogoCaja.aspx');
			}
			if(it.url=="./CatalogoParametro.aspx"){
				self.parent.navigate('./CatalogoParametro.aspx');
			}					
			if(it.url=="./CatalogoUsuario.aspx"){
				self.parent.navigate('./CatalogoUsuario.aspx');
			}					
			if(it.url=="./CatalogoConceptoID.aspx"){
				self.parent.navigate('./CatalogoConceptoID.aspx');
			}
			
			//MODULO DE FUNCIONES ESPECIALES
				
				//MODULO INGRESOS
				if(it.url=="./IngresosDelete.aspx"){				
					self.parent.navigate('./IngresosDelete.aspx');				   
				}			
			
			//MODULO DE INGRESOS
			if(it.url=="./Ingresos.aspx"){				
				self.parent.navigate('./Ingresos.aspx');				   
			}
			
			//MODULO DE BANCOS
			if(it.url=="./EstadoCuentaImportaRpt.aspx?TituloMenu=Bancos"){				
				//self.parent.navigate('./EstadoCuentaImportaRpt.aspx?TituloMenu=Bancos');
				alert('Lo sentimos, pagina en desarrollo.');
			}			
			if(it.url=="./EstadoCuentaConciliacion.aspx"){				
				//self.parent.navigate('./EstadoCuentaConciliacion.aspx');				   
				alert('Lo sentimos, pagina en desarrollo.');
			}
			...
			//MODULO DE REPORTES
				...
				//MODULO INGRESOS
				if(it.url=="./IngresosRpt.aspx?TituloMenu=PorRango"){				
					self.parent.navigate('./IngresosRpt.aspx?TituloMenu=Acumulado por rango de fechas');				   
				}			
				if(it.url=="./IngresosRpt.aspx?TituloMenu=Mensual"){				
					self.parent.navigate('./IngresosRpt.aspx?TituloMenu=Acumulado mensual');				   
				}
				if(it.url=="./IngresosDetalleRpt.aspx?TituloMenu=PorRango"){				
					self.parent.navigate('./IngresosDetalleRpt.aspx?TituloMenu=Detalle y Acumulado por rango de fechas');				   
				}
				if(it.url=="./DesgloseConceptoRpt.aspx"){				
					self.parent.navigate('./DesgloseConceptoRpt.aspx');				   
				}	
				if(it.url=="./DesgloseConceptoBusca.aspx"){				
					self.parent.navigate('./DesgloseConceptoBusca.aspx');				   
				}					
				if(it.url=="./DesgloseConceptoRptCheque.aspx"){				
					self.parent.navigate('./DesgloseConceptoRptCheque.aspx');					
				}	
				if(it.url=="./DesgloseConceptoRptDescuadre.aspx"){				
					self.parent.navigate('./DesgloseConceptoRptDescuadre.aspx');					
				}							
				
				//MODULO BANCOS
				if(it.url=="./EstadoCuentaImportaRpt.aspx?TituloMenu=Reportes"){				
					//self.parent.navigate('./EstadoCuentaImportaRpt.aspx?TituloMenu=Reportes');				   
					alert('Lo sentimos, pagina en desarrollo.');
				}	
						
			//SALIDA DEL SISTEMA
			if(it.url=="self.parent.navigate"){				
				window.close();
			}			
...
//solo permite la captura de números enteros
function fnNumeroInteger(){			
	if (event.keyCode < 48 || event.keyCode > 57) event.returnValue = false;
}

//funcion q obtiene el nùmero de cheque de la cadena q obtiene el lector
function fnNumeroCheque(obj){
	if(event.keyCode==13||event.keyCode==40){
		event.keyCode = 9;
		var codigo = obj.value;
		obj.value = obj.value.substring(codigo,24,7)
		//alert(obj.value);
		event.returnValue = true;
	}
}

//solo permite la captura de números enteros y decimales (el punto)
function fnNumeroDecimal(obj){
	if(event.keyCode == 46){
		var count = 0;
		var index = obj.value.indexOf(".")+2;
		if(index>1){
			++count;
		}
		if(count>0){
			if(obj.value.substring(index,obj.lenght-1).indexOf(".")!=-1){
...
//solo permite el enter, tecla hacia abajo
function fnEnter(){
	if(event.keyCode==13||event.keyCode==40){
		event.keyCode = 9;
		event.returnValue = true;
...
//solo permite el enter, tecla hacia abajo
function fnEnterGrid(obj){
	if(event.keyCode==13||event.keyCode==40){
		var control = obj.name.substr(15,obj.name.length-1);		
		var index = control.substring(0,control.indexOf(":"));		
		document.getElementById('dgrResumen__ctl' + index + '_btnGuardar').focus();		
	}
}

//solo permite el enter, tecla hacia abajo, tecla hacia la derecha, tecla hacia la izquierda, tecla hacia arriba
function fnEnterRepeater(obj){		
	...
	if(event.keyCode==38){
		...
		if(obj.name.indexOf("btnDesglose")==-1){		
			var control = obj.name.substr(20,obj.name.length-1);		
			var index = control.substring(0,control.indexOf(":"));
			var indexanterior = control.substring(0,control.indexOf(":")) - 1;
			if(index>0){
				var name = control.substring(control.indexOf(":")+1 ,control.length-control.indexOf(":")+control.substring(0,control.indexOf(":")).length);										
				if(name=='Monto'){
					name = 'Kilos';
				}else{
					name = 'Monto';
				}
				var objeto = 'RepeaterRenglon__ctl' + index + '_' + name;					
								
				if(document.getElementById(objeto)==null){
					name = 'Monto';
					objeto = 'RepeaterRenglon__ctl' + indexanterior + '_Monto';					
					var controlanterior = document.getElementById(objeto);		
					if (controlanterior.disabled == false){						
						controlanterior.focus();						
					}else{		
						while (indexanterior > 1){				
							indexanterior = indexanterior - 1;
							objeto = 'RepeaterRenglon__ctl' + indexanterior + '_Monto';
							controlanterior = document.getElementById(objeto);
							if (controlanterior.disabled == false){								
								controlanterior.focus();								
								indexanterior = 1;
							}
					...
					if(name=='Monto'){						
						if(indexanterior>0){
							objeto = 'RepeaterRenglon__ctl' + indexanterior + '_' + name;
							controlanterior = document.getElementById(objeto);							
							controlanterior.focus();							
						}
					}else{
						objeto = 'RepeaterRenglon__ctl' + index + '_' + name;
						controlanterior = document.getElementById(objeto);						
						controlanterior.focus();						
					}					
				}
...
function fnCalculoKilos(){	
	...
	var j = 1;
	var k = 1;
	var objetoKilos = 'RepeaterRenglon__ctl' + i + '_Kilos';
	var controlKilos = document.getElementById(objetoKilos);
	var objetoKilosRestar = 'RepeaterRenglon__ctl' + i + '_Kilos';
	var controlKilosRestar = document.getElementById(objetoKilosRestar);		
	var objetoTipoTotal = 'RepeaterRenglon__ctl' + i + '_TipoTotal';
	var controlTipoTotal = document.getElementById(objetoTipoTotal);	
	var objetoRestar = 'RepeaterRenglon__ctl' + i + '_Restar';
	var controlRestar = document.getElementById(objetoRestar);
	var objetoPosicion = 'RepeaterRenglon__ctl' + j + '_Posicion';
	var controlPosicion = document.getElementById(objetoPosicion);

	while(i<110){
		if(document.getElementById(objetoKilos)!=null){
			++k;
		}else{
			i = 110;
		}
		++i;
		objetoKilos = 'RepeaterRenglon__ctl' + i + '_Kilos';
	}

	i = 1;
	objetoKilos = 'RepeaterRenglon__ctl' + i + '_Kilos';
	while(i<k){	
		if(document.getElementById(objetoTipoTotal)!=null){	
		    controlTipoTotal = document.getElementById(objetoTipoTotal);
			if(controlTipoTotal.value=='False'){
				if(document.getElementById(objetoKilos)!=null){
					controlKilos = document.getElementById(objetoKilos);
					controlRestar = document.getElementById(objetoRestar);
					valorRestar = controlRestar.value.toUpperCase();
					if(valorRestar.substring(0,1)=='S'){						
						posRestar = valorRestar.substring(1,valorRestar.length);
						j = 1;						
						while(j<k){							
							objetoPosicion = 'RepeaterRenglon__ctl' + j + '_Posicion';
							if(document.getElementById(objetoPosicion)!=null){
								controlPosicion = document.getElementById(objetoPosicion);
								if(controlPosicion.value==posRestar){
									posRestar = j;
									j = k;																											
								}
							}
						...
						objetoKilosRestar = 'RepeaterRenglon__ctl' + posRestar + '_Kilos';
						controlKilosRestar = document.getElementById(objetoKilosRestar);						
						controlKilos.value = controlKilosRestar.value;
						if(controlKilos.value>0){				
							TotalKilosNeto = parseInt(TotalKilosNeto) + parseInt(controlKilos.value);
						}						
					}else{
						if(controlKilos.value>0){				
							TotalKilos = parseInt(TotalKilos) + parseInt(controlKilos.value);
						}	
					}					
				}else{
					i = k;
				...
				if(document.getElementById(objetoKilos)!=null){
					controlKilos = document.getElementById(objetoKilos);				
					if(TotalKilosNeto>0){
						controlKilos.value = TotalKilos - TotalKilosNeto;
					}else{
						controlKilos.value = TotalKilos;
					}
				}
			}
		}			
		++i;
		objetoKilos = 'RepeaterRenglon__ctl' + i + '_Kilos';
		objetoTipoTotal = 'RepeaterRenglon__ctl' + i + '_TipoTotal';
		objetoRestar = 'RepeaterRenglon__ctl' + i + '_Restar';		
	}
}

function fnCalculoMonto(obj){

	var control = obj.name.substr(20,obj.name.length-1);		
	var index = control.substring(0,control.indexOf(":"));	
	var objetoTipo = 'RepeaterRenglon__ctl' + index + '_Tipo';
		
	if(document.getElementById(objetoTipo).value.toUpperCase()=='INGRESO'){
		fnCalculoIngreso();		
	}else{
		fnCalculoDeposito();				
	}	
...
function fnCalculoIngreso(){
	var TotalMonto = 0.0;	
	var TotalMontoNeto = 0.0;
	var valorRestar = '';	
	var posRestar = '';	
	var i = 1;
	var j = 1;
	...
	var objetoKilos = 'RepeaterRenglon__ctl' + i + '_Kilos';
	var controlKilos = document.getElementById(objetoKilos);
	var objetoMonto = 'RepeaterRenglon__ctl' + i + '_Monto';
	var controlMonto = document.getElementById(objetoMonto);
	var objetoMontoRestar = 'RepeaterRenglon__ctl' + i + '_Monto';
	var controlMontoRestar = document.getElementById(objetoMontoRestar);
	var objetoTipoTotal = 'RepeaterRenglon__ctl' + i + '_TipoTotal';
	var controlTipoTotal = document.getElementById(objetoTipoTotal);
	var objetoRestar = 'RepeaterRenglon__ctl' + i + '_Restar';
	var controlRestar = document.getElementById(objetoRestar);
	var objetoPosicion = 'RepeaterRenglon__ctl' + j + '_Posicion';
	var controlPosicion = document.getElementById(objetoPosicion);
	var objetoTipo = 'RepeaterRenglon__ctl' + i + '_Tipo';
	var controlTipo = document.getElementById(objetoTipo);
	...
	while(i<k){			
		if(document.getElementById(objetoTipoTotal)!=null){	
		    controlTipoTotal = document.getElementById(objetoTipoTotal);
			if(controlTipoTotal.value=='False'){
				if(document.getElementById(objetoMonto)!=null){
					controlMonto = document.getElementById(objetoMonto);
					controlRestar = document.getElementById(objetoRestar);					
					valorRestar = controlRestar.value.toUpperCase();
					controlTipo = document.getElementById(objetoTipo);
					controlTipo = controlTipo.value.toUpperCase();					
					if(valorRestar.substring(0,1)=='S'){						
						posRestar = valorRestar.substring(1,valorRestar.length);						
						j = 1;
						objetoPosicion = 'RepeaterRenglon__ctl' + j + '_Posicion';
						...
						objetoMontoRestar = 'RepeaterRenglon__ctl' + posRestar + '_Monto'
						controlMontoRestar = document.getElementById(objetoMontoRestar);						
						controlMonto.value = controlMontoRestar.value;
						if(controlMonto.value>0.0){
							TotalMontoNeto = parseFloat(TotalMontoNeto) + parseFloat(controlMonto.value);
						}
					}else{
						if(controlMonto.value>0.0){	
							TotalMonto = parseFloat(TotalMonto) + parseFloat(controlMonto.value);					
						}	
					}					
				}		
			}else{
				if(document.getElementById(objetoMonto)!=null){
					controlMonto = document.getElementById(objetoMonto);																
					if(TotalMontoNeto>0.0){
						controlMonto.value = parseFloat(TotalMonto) - parseFloat(TotalMontoNeto);
					}else{
						controlMonto.value = parseFloat(TotalMonto);
					}					
					controlMonto.value = (parseInt(controlMonto.value*1000))/1000;					
				}
			}
		}			
		++i;
		objetoMonto = 'RepeaterRenglon__ctl' + i + '_Monto';
		objetoTipoTotal = 'RepeaterRenglon__ctl' + i + '_TipoTotal';
		objetoRestar = 'RepeaterRenglon__ctl' + i + '_Restar';
		objetoTipo = 'RepeaterRenglon__ctl' + i + '_Tipo';
	}
}

function fnCalculoDeposito(){
	var TotalMonto = 0.0;	
	var TotalMontoNeto = 0.0;
	var valorRestar = '';	
	var posRestar = '';	
...
//No permite el caracter comilla simple ' ni el menor que <
function fnCaracterEspecial(){
	if (event.keyCode == 39 || event.keyCode == 60) event.returnValue = false;
}

//cambia el estilo del objeto al recibir el foco personalizado
function fnStylefocusRepeater(obj){	
	var control = obj.name.substr(20,obj.name.length-1);
	var index = control.substring(0,control.indexOf(":"));		
	var objetoTipoTotal = 'RepeaterRenglon__ctl' + index + '_TipoTotal';
	var objetoRestar = 'RepeaterRenglon__ctl' + index + '_Restar';
				
	if(document.getElementById(objetoTipoTotal)!=null){	
	    var controlTipoTotal = document.getElementById(objetoTipoTotal);
		if(controlTipoTotal.value=='False'){			
			var controlRestar = document.getElementById(objetoRestar);					
			var	valorRestar = controlRestar.value.toUpperCase();
			if(valorRestar.substring(0,1)=='N'){							
				obj.style.backgroundColor='Honeydew';
				obj.style.color='red';
...
//cambia el estilo del objeto al perder el foco personalizado
function fnStyleblurRepeater(obj){
	var control = obj.name.substr(20,obj.name.length-1);
	var index = control.substring(0,control.indexOf(":"));		
	var objetoTipoTotal = 'RepeaterRenglon__ctl' + index + '_TipoTotal';
	var objetoRestar = 'RepeaterRenglon__ctl' + index + '_Restar';
...
//cambia el estilo del objeto al recibir el foco
function fnStylefocus(obj){	
	obj.style.backgroundColor='Honeydew';
	obj.style.color='red';
	obj.style.fontWeight='bold'	
}

//cambia el estilo del objeto cheque al recibir el foco
function fnStylefocusCheque(obj){	
	obj.style.backgroundColor='Honeydew';
	obj.style.color='gray';
	//obj.style.fontWeight='bold'	
}

//cambia el estilo del objeto al perder el foco
function fnStyleblur(obj){
	obj.style.backgroundColor='white';
	obj.style.color='#000099';
	obj.style.fontWeight='normal'
}

//inhabilitar el botón derecho del mouse
function enabledmouseright(e){
	if (navigator.appName == 'Netscape' && (e.which == 3 || e.which == 2)){		
		return false;
	}else{
		 if (navigator.appName == 'Microsoft Internet Explorer' && (event.button == 2 || event.button == 3)){
			alert("Lo siento, el boton derecho del raton esta inhabilitado");
			return false;
		}
	}
	return true;
...
//sólo permite números y diagonales "/"
function fnCaracterFecha(){	
	if (event.keyCode < 47 || event.keyCode > 57) event.returnValue = false;	
}

//valida fecha
function fnFecha(obj){

	var Len = parseInt(obj.value.length);
	var d1 = obj.value.substr(2,1);
	var d2 = obj.value.substr(5,1);
	var d = obj.value.substr(0,2);	
	var m = obj.value.substr(3,2);
	var a = obj.value.substr(6,4);
	if(Len>0){				
		if(Len!=10 || d1!='/' || d2!='/' || d==0 || d>31 || m==0 || m>12 || a<1970 || a>2100){
			fnError(obj);		
		}
	}
...
//manda mensaje de error y regresa foco a objeto
function fnError(obj){
	var control = obj.name.substr(15,obj.name.length-1);		
	var index = control.substring(0,control.indexOf(":"));	
	document.getElementById('dgrResumen__ctl' + index + '_FChequeEdit').focus();
	alert('Fecha incorrecta. Formato dd/mm/aaaa, verifique');
}

// Suma el monto capturado por tipo de movimiento de desglose concepto
function fnSuma(obj){

	var control = obj.name.substr(15,obj.name.length-1);
	var index = control.substring(0,control.indexOf(":"));		
	var objetoTipo = 'dgrResumen__ctl' + index + '_TipoEdit';
	var controlTipo = document.getElementById(objetoTipo);		
	var movimiento = controlTipo.value;	
	var mov = 0
	
	if(movimiento>0){
		var j = 2;
		var objetoMov = 'dgrMovimiento__ctl' + j + '_Clave';
		var controlMov = document.getElementById(objetoMov);
		var mov = controlMov.value;
			
		while(movimiento!=mov){
			j++
			objetoMov = 'dgrMovimiento__ctl' + j + '_Clave';
			controlMov = document.getElementById(objetoMov);
			mov = controlMov.value;
		}
		var objetoTipoMov = 'dgrMovimiento__ctl' + j + '_TipoMovimiento';
		var controlTipoMov = document.getElementById(objetoTipoMov);
		var TipoMov = controlTipoMov.value;
		var objetoMonto = 'dgrMovimiento__ctl' + j + '_Monto';
		var controlMonto = document.getElementById(objetoMonto);
		var monto = controlMonto.value;
		var objetoDisponible = 'dgrMovimiento__ctl' + j + '_Disponible';
		var controlDisponible = document.getElementById(objetoDisponible);	
		var disponible = controlDisponible.value;
							
		var suma = 0.0;
		var valor = 0.0;
		var i = 2;
		var objeto = 'dgrResumen__ctl' + i + '_MontoEdit';
		var control = document.getElementById(objeto);
					
		objetoTipo = 'dgrResumen__ctl' + i + '_TipoEdit';
		controlTipo = document.getElementById(objetoTipo);
		
		while(i<170){
			if(movimiento==controlTipo.value){
				if(control.value>0.0){
					valor = control.value;
				}else{
					valor = 0.0;
				}				
				suma = suma + parseInt(valor*1000)/1000;
			}
			i++;
			objeto = 'dgrResumen__ctl' + i + '_MontoEdit';
			control = document.getElementById(objeto);
			objetoTipo = 'dgrResumen__ctl' + i + '_TipoEdit';
			controlTipo = document.getElementById(objetoTipo);
		}		
		
		var diferencia = parseInt(monto*1000)/1000 - parseInt(suma*1000)/1000;		
		diferencia = parseInt(diferencia*1000)/1000;
		
		if(diferencia=="0.001" || diferencia=="0.002" || diferencia=="0.003" || diferencia=="0.004" || diferencia=="0.005" || diferencia=="0.006" || diferencia=="0.007" || diferencia=="0.008" || diferencia=="0.009" || diferencia=="-0.001" || diferencia=="-0.002" || diferencia=="-0.003" || diferencia=="-0.004" || diferencia=="-0.005" || diferencia=="-0.006" || diferencia=="-0.007" || diferencia=="-0.008" || diferencia=="-0.009"){
			diferencia = 0.00
		}
				
		controlDisponible.value = diferencia;
		if(diferencia<0){			
			alert('Diferencia de ' + TipoMov + ' ' + diferencia + ', verifique');
			objetoFoco = 'dgrResumen__ctl' + index + '_MontoEdit';
			document.getElementById(objetoFoco).focus();		
		}		
	}
}
